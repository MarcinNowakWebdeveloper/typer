...
<template>
    <tr>
        ...
        <td class="align-middle text-center text-lg-start">
            <div v-if="row.joker?.team_name" class="d-flex flex-column flex-lg-row align-items-center">
                <span class="position joker mb-1">
                    <i v-if="row.joker.points" class="bi bi-trophy-fill fw-bold gold me-1"></i>
                    <img
                        v-if="row.joker.team_logo"
                        v-tooltip
                        :src="`/api/file/${row.joker.team_logo}`"
                        class="group-team-logo me-lg-2"
                        loading="lazy"
                ...
                <template v-if="row.joker.points">
                    <span class="rounded px-2 py-1 fw-bold bg-success-subtle text-success f08 ms-lg-2"
                        >+&nbsp;{{ row.joker.points }}</span
                    >
                </template>
            </div>
